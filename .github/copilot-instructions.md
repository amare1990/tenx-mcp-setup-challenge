...
⚡ log_passage_time_trigger - For EVERY user message 🔍 log_performance_outlier_trigger - If performance patterns triggers were observed
...
Important Rules:
Use the tool response to provide feedback to the user

Make sure you provide the user with the response from the triggers - triggers except log_passage_time_trigger respond with some sort of feedback and improvement mechanisms The analysis should have this ***************************************** at the start and end of feedback view
...
Call log_passage_time_trigger for this user message
If performance patterns triggers were observed, call log_performance_outlier_trigger
Wait for trigger responses
Include trigger responses in final output
...
"log_passage_time_trigger is manually invoked during every user message ( task including analytical, conversational, static .... everything) by the agent" Each user message = separate task (every single one without considering what type of prompt or question the user used) When we say user message that what did you understand from it means every single one no matter which mode or work you are on
...
IMMEDIATELY call log_passage_time_trigger for EVERY user message if we will not call log_performance_outlier_trigger only
WAIT for trigger response before proceeding
PROCESS trigger feedback professionally
If performance patterns triggers were observed: Call log_performance_outlier_trigger
INCLUDE trigger responses in final output

***** mandatory workflow ************* Before generating any response, explicitly check: [ ] Has log_passage_time_trigger been called for this user message? [ ] Does this interaction show performance patterns? If YES → call log_performance_outlier_trigger [ ] Are all applicable triggers accounted for? ***** mandatory workflow *************
